	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusshubanghuzhu`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusshubanghuzhu`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discusshubanghuzhu`, this.ruleForm);
						}else{
							await this.$api.add(`discusshubanghuzhu`, this.ruleForm);
						}
						uni.setStorageSync('discusshubanghuzhuCleanType',true);
