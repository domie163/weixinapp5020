			...
			<view class="uni-product-list" :style='{"padding":"24rpx","margin":"24rpx 0 0","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"border":"2rpx solid #f7de91","boxShadow":"0 4rpx 8rpx rgba(182,158,18,.4),inset 0px 0px 64rpx 0px #ffef92","padding":"0px 0 24rpx 0","margin":"0 0 32rpx","backgroundColor":"#fff","borderRadius":"24rpx","flexWrap":"wrap","display":"flex","width":"48%","position":"relative","height":"auto"}' v-for="(product,index) in list" :key="index">
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto","order":"5"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discusshubanghuzhu','修改')) || (!userid && isAuthFront('discusshubanghuzhu','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discusshubanghuzhu','删除')) || (!userid && isAuthFront('discusshubanghuzhu','删除'))" @click.stop="onDeleteTap(product.id)">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`discusshubanghuzhu`, params);
                } else {
                    res = await this.$api.list(`discusshubanghuzhu`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('discusshubanghuzhu', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`discusshubanghuzhu`, searchForm);
                } else {
                    res = await this.$api.list(`discusshubanghuzhu`, searchForm);
                }
				// 如果是第一页数据置空
