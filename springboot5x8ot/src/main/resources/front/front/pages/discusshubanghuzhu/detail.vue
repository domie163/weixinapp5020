...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusshubanghuzhuCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusshubanghuzhuCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusshubanghuzhu');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discusshubanghuzhu');
			...
			async init(){
				...
				let res = await this.$api.info('discusshubanghuzhu', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusshubanghuzhu', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscusshubanghuzhu/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
