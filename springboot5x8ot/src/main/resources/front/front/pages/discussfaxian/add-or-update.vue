	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussfaxian`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussfaxian`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussfaxian`, this.ruleForm);
						}else{
							await this.$api.add(`discussfaxian`, this.ruleForm);
						}
						uni.setStorageSync('discussfaxianCleanType',true);
