...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussfaxianCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussfaxianCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussfaxian');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussfaxian');
			...
			async init(){
				...
				let res = await this.$api.info('discussfaxian', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussfaxian', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussfaxian/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
