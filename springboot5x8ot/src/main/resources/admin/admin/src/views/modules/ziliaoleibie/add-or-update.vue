<template>
	<div class="addEdit-block" :style='{"padding":"0"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 2% 16px 0","background":"none","display":"inline-block"}' class="input" v-if="type!='info'"  label="资料类别" prop="ziliaoleibie">
					<el-input v-model="ruleForm.ziliaoleibie" placeholder="资料类别" clearable  :readonly="ro.ziliaoleibie"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 2% 16px 0","background":"none","display":"inline-block"}' v-else class="input" label="资料类别" prop="ziliaoleibie">
					<el-input v-model="ruleForm.ziliaoleibie" placeholder="资料类别" readonly></el-input>
				</el-form-item>
			</template>
...
export default {
	data() {
		...
		return {
			...
			rules: {
				ziliaoleibie: [
					{ required: true, message: '资料类别不能为空', trigger: 'blur' },
				],
			}
		};
	},
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `ziliaoleibie/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.ziliaoleibieCrossAddOrUpdateFlag = false;
						 this.parent.search();
